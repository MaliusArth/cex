// Reference: Unreal Engine (GenericPlatform.h)
...
namespace ma
{
	...
	{
		// nothing here; error occurs if the partial specializations fail
	};
	...
	struct GenericPlatformTypes
	{
		...
		typedef SelectIntPointerType<uint32, uint64, sizeof(void*)>::IntPointer UPTRINT;	// unsigned int the same size as a pointer
		typedef SelectIntPointerType<int32, int64, sizeof(void*)>::IntPointer PTRINT;		// signed int the same size as a pointer
