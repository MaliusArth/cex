...
#include <malius/preprocessor.hpp>  // preprocessor
//#include <malius/preprocessor/stringize.hpp>  // preprocessor
#include <malius/preprocessor/variadic/size.hpp>  // preprocessor
...
namespace ma
{
    ...
    IF(EQUAL( MA_PP_VARIADIC_SIZE( MA_PP_REMOVE_PARENS(x)), 2)\
        ( MA_PP_GLUE(op, ( MA_PP_REMOVE_PARENS(x),  MA_DECLARE_FLAGS_GET_VALUE( MA_PP_REMOVE_PARENS(x)))),\
         MA_PP_GLUE(op, ( MA_PP_REMOVE_PARENS(x), count)))\
         MA_PP_EAT(__VA_ARGS__)*/
//#define  MA_DECLARE_FLAGS_ENUM(i, name)      name = (1u << i),
#define  MA_DECLARE_FLAGS_ENUM(i, name)      name = (1u << i),
#define  MA_DECLARE_FLAGS_BITS(name)      uint32_t name : 1;
#define  MA_DECLARE_FLAGS_TO_STRING(name) \
case name: return  MA_PP_STRINGIZE(name);

#define  MA_DECLARE_FLAGS(name, ...)    \
struct name     \
{       \
    static const size_t Count =  MA_PP_VARIADIC_SIZE(__VA_ARGS__);     \
    static_assert(Count <= 32 /*sizeof(uint32_t)*8*/, "Too many flags used in  MA_DECLARE_FLAGS. A maximum number of 32 flags is allowed.");     \
    enum Enum       \
    {   \
        /* MA_PP_FOR_EACH_ITER( MA_DECLARE_FLAGS_ENUM, __VA_ARGS__)*/  \
        /* MA_PP_FOR_EACH_SIMPLE( MA_DECLARE_FLAGS_ENUM, __VA_ARGS__)*/\
         MA_PP_FOR_EACH_SIMPLE_ITER( MA_DECLARE_FLAGS_ENUM, __VA_ARGS__)\
    };  \
    struct Bits     \
    {   \
...
{   \
    ...
    Flags<name> flags(lhs);\
    return std::move(flags |= rhs);    \
...
template <class T>
class /* MA_API*/ Flags  //  symbol exporting not needed for inline (header-only) classes, functions etc.
{
...
/// TODO: extend Flags<T> with more functionality from Flags<T> from Vulkan https://github.com/KhronosGroup/Vulkan-Hpp/blob/malius/vulkan/vulkan.hpp
